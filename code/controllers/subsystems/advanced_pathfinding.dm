SUBSYSTEM_DEF(advanced_pathfinding)
	...
	priority = SS_PRIORITY_APF
	init_order = SS_INIT_APF
	wait = 1 SECONDS
...
/obj/effect/ai_node/goal/Initialize(mapload, mob/creator)
	...
	if(!creator)
		return
	src.creator = creator
	register_signal(creator, SIGNAL_QDELETING, nameof(.proc/clean_creator))
	goal_image = image('icons/effects/landmarks_static.dmi', src, "no name")
