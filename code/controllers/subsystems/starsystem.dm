...
SUBSYSTEM_DEF(star_system)
	...
	var/list/contested_systems = list()	//A maintained list containing all systems that have fleets of opposing factions in them.
...
/datum/controller/subsystem/star_system/Initialize(start_timeofday)
	...
	for(var/datum/star_system/S in systems)	//Setup the neutral zone for easier access
		if(S.sector != SECTOR_NEUTRAL)
			continue

		neutral_zone_systems += S

/// Overmap combat events control system, adds weight to combat events over time spent out of combat
/datum/controller/subsystem/star_system/fire()
	pass()
