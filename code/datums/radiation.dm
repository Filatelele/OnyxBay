...
/proc/generate_rad_resist_cache()
	...
	for(var/datum/rad_resist/resist_type in subtypesof(/datum/rad_resist))
		resist_type = new resist_type()
		resist_cache[resist_type.type] = resist_type
	return resist_cache
...
/proc/get_rad_resist(resist_type)
	...
	var/datum/rad_resist/resist = locate(replacetext("[resist_type]", "/", "-")) in GLOB.rad_resists_cache
	if(istype(resist))
		return resist
