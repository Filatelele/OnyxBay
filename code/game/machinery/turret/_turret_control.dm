...
/obj/machinery/turretcp/tgui_data(mob/user)
	...
	for(var/obj/machinery/turret/T in connected_turrets)
		var/obj/installed_gun = T.installed_gun
		var/list/turret_data = list(
			"gun" = installed_gun?.name,
		...
		if(istype(installed_gun, /obj/item/gun/projectile))
			...
			turret_data["ammo"] = stored_magazine?.stored_ammo?.len
...
/obj/machinery/turretcp/proc/update_turrets()
	...
	for(var/obj/machinery/turret/network/T in turrets)
		...
		T.check_lifeforms = check_anomalies
		T.enabled = enabled
