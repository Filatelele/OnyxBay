/mob/living/carbon/human/verb/give_verb(mob/living/target in view(1)-usr)
	set category = "IC"
	set name = "Give"

	give(target)

/mob/living/carbon/human/proc/give(mob/living/target)
	if(incapacitated())
		return
	...
	if(usr.drop(I))
		...
		usr.visible_message(SPAN_NOTICE("\The [usr] handed \the [I] to \the [target]."))
