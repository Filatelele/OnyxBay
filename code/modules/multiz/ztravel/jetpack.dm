...
/datum/vertical_travel_method/jetpack/start_animation()
	...
	if (direction == DOWN)
		...
		M.plane = TURF_PLANE
		M.layer = 1
		animate(M, alpha = 100, pixel_y = -8, transform = mat,  time = duration * 1.2, easing = SINE_EASING, flags = ANIMATION_END_NOW)
	else
		animate(M, alpha = 100, pixel_y = 32, time = duration * 1.2, easing = SINE_EASING, flags = ANIMATION_END_NOW)
...
/datum/vertical_travel_method/jetpack/can_perform(dir)
	. = ..()
	if(.)
		if(!istype(thrust))
...
/datum/vertical_travel_method/jetpack/tick()
	...
	if (world.time >= next_burst)
		...
		if(thrust.allow_thrust(JETPACK_MOVE_COST * burst_interval * 0.3, M, TRUE))
			var/obj/effect/effect/E = thrust.ion_trail.start()
