...
/obj/screen/alert/overmap_integrity
	name = "Ship integrity"
	icon = 'icons/overmap/armour_quadrants.dmi'
	icon_state = "base"
*/
...
/obj/structure/overmap/proc/full_repair() //Admin Override
	integrity = max_integrity //Full structural integrity
...
/obj/structure/overmap/proc/repair_structure(input, failure = 0) //Input is the amount you want repaired per call of this proc, failure is the chance the repair will go wrong
	...
	//	//new /obj/effect/overmap/explosion_telegraph(T, damage_amount = input)
	//	return
	...
	if(structure_crit) //Check for structural crit
		if(integrity >= max_integrity * 0.2) //End structural crit if high enough
			stop_relay(channel = SOUND_CHANNEL_AMBIENT)
			//priority_announce("Ship structural integrity restored to acceptable levels. ","Automated announcement ([src])")
			structure_crit = FALSE

/obj/structure/overmap/proc/repair_quadrant(input, failure = 0, bias = 50, quadrant) //Input is the amount you want repaired per call of this proc, failure is the chance the repair will go wrong, bias is the favour for structure damage vs armour damage
	var/percentile = input / 100
...
/obj/structure/overmap/proc/repair_all_quadrants(input, failure = 0, bias = 50) //Input is the amount you want repaired per call of this proc, failure is the chance the repair will go wrong, bias is the favour for structure damage vs armour damage
	var/percentile = input / 100
	if(use_armour_quadrants)
