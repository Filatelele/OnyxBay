...
/obj/item/gun/custom_ka/frameB/handle_post_fire(mob/user)
	...
	if(ishuman(user))
		...
		if(H.mob_size < 10) // smaller than an unathi
			...
			if(active_hand)
				LH.take_external_damage(10, 10)
			else
				RH.take_external_damage(10, 10)
...
/obj/item/gun/custom_ka/frameC/Fire(atom/target, atom/movable/firer, clickparams, pointblank = FALSE, reflex = FALSE, target_zone = BP_CHEST)
	//if(!isvaurca(user))
