		...
		if(prob(50))
			...
			if(W)
				...
				if(damage_eyes)
					...
					eyes.take_internal_damage(rand(1, 10), TRUE)
				else
					...
					holder.owner.take_overall_damage(rand(2, 10))
...
/datum/breakdown/negative/delusion/update()
	...
	if(prob(10))
		var/power = rand(9,27)
		holder.owner.playsound_local(holder.owner, 'sound/effects/ship/damage/explosionfar_6.ogg', 100, 1, round(power * 2, 1) )
...
/datum/breakdown/negative/fabric/occur()
	//register_signal(SSdcs, COMSIG_GLOB_FABRIC_NEW, nameof(.proc/add_image))
	register_signal(holder.owner, SIGNAL_LOGGED_IN, nameof(.proc/update_client_images))
...
/datum/breakdown/negative/fabric/conclude()
	...
	//unregister_signal(SSdcs, COMSIG_GLOB_FABRIC_NEW)
	unregister_signal(holder.owner, SIGNAL_LOGGED_IN)
	images.Cut()
	..()
...
/datum/breakdown/common/power_hungry/occur()
	//register_signal(holder.owner, COMSIG_CARBON_ELECTROCTE, nameof(.proc/check_shock))
	//register_signal(holder.owner, COMSIG_LIVING_STUN_EFFECT, nameof(.proc/check_shock))
	return ..()
...
/datum/breakdown/common/power_hungry/conclude()
	...
	//unregister_signal(holder.owner, COMSIG_LIVING_STUN_EFFECT)
	..()
...
/datum/breakdown/negative/glassification/can_occur()
	var/list/candidates = (GLOB.player_list & GLOB.living_mob_list_ & GLOB.human_mob_list) - holder.owner
	if(candidates.len)
		return TRUE
	return FALSE
...
/datum/breakdown/negative/glassification/update()
	...
	if(!.)
		return FALSE
	var/list/targets = (GLOB.player_list & GLOB.living_mob_list_ & GLOB.human_mob_list) - holder.owner
	if(targets.len)
		target = pick(targets)
		...
		//addtimer(CALLBACK(src, nameof(.proc/reset_views), TRUE), time_view)
...
/datum/breakdown/common/desire_for_chrome/occur()
	//register_signal(holder.owner, COMSIG_HUMAN_ROBOTIC_MODIFICATION, nameof(.proc/check_organ))
	return ..()

/datum/breakdown/common/desire_for_chrome/conclude()
	//unregister_signal(holder.owner, COMSIG_HUMAN_ROBOTIC_MODIFICATION)
	..()
...
/datum/breakdown/common/signs/occur()
	register_signal(holder.owner, SIGNAL_HUMAN_SAY, nameof(.proc/check_message))
	return ..()

/datum/breakdown/common/signs/conclude()
	unregister_signal(holder.owner, SIGNAL_HUMAN_SAY)
	..()
