...
#define SANITY_DAMAGE_VIEW(damage, will, dist) ((damage) * SANITY_VIEW_DAMAGE_MOD * (1.2 - (will) / STAT_LEVEL_MAX) * (1 - (dist)/15))

// Damage received from body damage
#define SANITY_DAMAGE_HURT(damage, will) (min((damage) / 5 * SANITY_DAMAGE_MOD * (1.2 - (will) / STAT_LEVEL_MAX), 60))

// Damage received from shock
#define SANITY_DAMAGE_SHOCK(shock, will) ((shock) / 50 * SANITY_DAMAGE_MOD * (1.2 - (will) / STAT_LEVEL_MAX))

// Damage received from psy effects
#define SANITY_DAMAGE_PSY(damage, will) (damage * SANITY_DAMAGE_MOD * (2 - (will) / STAT_LEVEL_MAX))

// Damage received from seeing someone die
#define SANITY_DAMAGE_DEATH(will) (10 * SANITY_DAMAGE_MOD * (1 - (will) / STAT_LEVEL_MAX))
...
/datum/sanity/New(mob/living/carbon/human/H)
	...
	level = max_level
	insight = rand(0, 30)
	register_signal(owner, SIGNAL_MOB_LIFE, nameof(.proc/on_life))
	register_signal(owner, SIGNAL_HUMAN_SAY, nameof(.proc/on_say))

/datum/sanity/Destroy()
	unregister_signal(owner, SIGNAL_MOB_LIFE)
	unregister_signal(owner, SIGNAL_HUMAN_SAY)
	owner = null
...
/datum/sanity/proc/on_life()
	//SIGNAL_HANDLER
	handle_breakdowns()
	if(owner.is_ic_dead() || owner.life_tick % life_tick_modifier || owner.InStasis() || (owner.species.lower_sanity_process && !owner.client))
		return
...
/datum/sanity/proc/handle_view()
	...
	for(var/atom/A in view(owner.client ? owner.client : owner))
		...
		//		. += SANITY_DAMAGE_VIEW(0.1, will, get_dist(owner, A))
...
/datum/sanity/proc/handle_area()
	...
	if(. < 0)
		. *= owner.stats.getStat(STAT_WILL) / STAT_LEVEL_MAX
...
/datum/sanity/proc/handle_level()
	...
	if(level < 40 && world.time >= spook_time)
		...
		var/static/list/effects_40 = list(
			nameof(.proc/effect_emote) = 25,
			nameof(.proc/effect_quote) = 50
		)
		var/static/list/effects_30 = effects_40 + list(
			nameof(.proc/effect_sound) = 1,
			nameof(.proc/effect_whisper) = 25
		)
		var/static/list/effects_20 = effects_30 + list(
			nameof(.proc/effect_hallucination) = 30
		)

		call(src, util_pick_weight(level < 30 ? level < 20 ? effects_20 : effects_30 : effects_40))()
...
/datum/sanity/proc/onDamage(amount)
	changeLevel(-SANITY_DAMAGE_HURT(amount, owner.stats.getStat(STAT_WILL)))

/datum/sanity/proc/onPsyDamage(amount)
	changeLevel(-SANITY_DAMAGE_PSY(amount, owner.stats.getStat(STAT_WILL)))

/datum/sanity/proc/onSeeDeath(mob/M)
	if(ishuman(M))
		var/penalty = -SANITY_DAMAGE_DEATH(owner.stats.getStat(STAT_WILL))
		...
		//		if(75 to 100)
		//			penalty *= 0
		//if(M.stats.getPerk(PERK_TERRIBLE_FATE) && prob(100-owner.stats.getStat(STAT_WILL)))
		//	setLevel(0)
		//else
		changeLevel(penalty * death_view_multiplier)

/datum/sanity/proc/onShock(amount)
	changeLevel(-SANITY_DAMAGE_SHOCK(amount, owner.stats.getStat(STAT_WILL)))

/datum/sanity/proc/onDrug(datum/reagent/R, multiplier)
	changeLevel(R.sanity_gain * multiplier)
	if(resting)
		add_rest(INSIGHT_DESIRE_DRUGS, 4 * multiplier)

/datum/sanity/proc/onToxin(datum/reagent/R, multiplier)
...
/datum/sanity/proc/onReagent(datum/reagent/E, multiplier)
	...
	if(istype(E, /datum/reagent/ethanol))
...
/datum/sanity/proc/onEat(obj/item/reagent_containers/food/snack, snack_sanity_gain, snack_sanity_message)
...
/datum/sanity/proc/breakdown(positive_breakdown = FALSE)
	...
	while(possible_results.len)
		...
		if(B.occur())
			breakdowns += B
			//for(var/mob/living/carbon/human/H in viewers(owner))
			//	SEND_SIGNAL(H, SIGNAL_HUMAN_BREAKDOWN, owner, B)
		return
